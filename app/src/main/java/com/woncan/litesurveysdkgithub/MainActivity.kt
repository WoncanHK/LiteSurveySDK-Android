...
class MainActivity : AppCompatActivity() {
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		binding.btnRefreshMountpoint.setOnClickListener {
			...
			LiteSurveyDevice.queryNtripMountpoint(
				address,
				port,
	...
	private fun scan() {
		LiteSurveyDeviceScanner.startScan(this, object : LiteSurveyDeviceListener() {
			...
			override fun onBatteryInfo(device: LiteSurveyDevice, batteryInfo: BatteryInfo) {
				runOnUiThread {
			...
			override fun onFirmwareUpgradeAvailability(newFirmwareAvailable: Boolean) {
				runOnUiThread {
					binding.tvFirmwareInfo.text = String.format(
						Locale.getDefault(),
			...
			override fun onLocationChanged(location: LiteSurveyLocation) {
				runOnUiThread {
					binding.tvStatus.text = when (location.fixStatus) {
						GnssFixStatus.Unknown -> "Unknown"
						GnssFixStatus.Single -> "Single"
						GnssFixStatus.DGNSS -> "DGNSS"
						GnssFixStatus.Fixed -> "Fixed"
						GnssFixStatus.Float -> "Float"
						GnssFixStatus.Simulated -> "Simulated"
					}
